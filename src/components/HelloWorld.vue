...
<v-col cols="6">
  <v-card append-icon="mdi-open-in-new" class="py-4" color="surface-variant" href="https://vuetifyjs.com/components/all"
    prepend-icon="mdi-widgets-outline" rel="noopener noreferrer" rounded="lg"
    subtitle="Discover components in the API Explorer." target="_blank" title="Components" variant="text">
    <v-overlay opacity=".06" scrim="primary" contained model-value persistent />
  </v-card>
</v-col>
  ...
  <v-card append-icon="mdi-open-in-new" class="py-4" color="surface-variant" href="https://discord.vuetifyjs.com"
    ...
    <v-container>
      ...
      <v-row>
        ...
        <v-col cols="8">
          <v-text-field label="手機" outlined v-model="name" />
        </v-col>
        <v-col cols="4">
          <v-btn @click="submitForm" color="primary" depressed>
            發送OTP驗證碼
          </v-btn>
        </v-col>
 
        <v-col cols="12">
          <v-text-field label="驗證碼" outlined v-model="email" />
        </v-col>
        <v-col cols="12">
          <v-btn @click="submitForm" color="primary" depressed>
            Submit
